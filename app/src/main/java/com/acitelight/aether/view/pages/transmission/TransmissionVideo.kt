...
import com.acitelight.aether.view.components.video.VideoDownloadCard
import com.acitelight.aether.viewModel.transmission.TransmissionScreenViewModel
import com.tonyodev.fetch2.Status
import java.io.File

@Composable
fun TransmissionVideo(
...
)
{
    ...
    val downloads = transmissionScreenViewModel.downloadVideos
    ...
    Column(modifier = Modifier.animateContentSize())
    {
        ...
        Text(
            text = "All: ${downloads.count { it.type == DownloadType.VideoMainFile }}",
            modifier = Modifier
        ...
        Text(
            text = "Completed: ${downloads.count { it.type == DownloadType.VideoMainFile && it.status == Status.COMPLETED }}",
            modifier = Modifier
                .padding(horizontal = 8.dp)
                .align(Alignment.Start),
            fontSize = 12.sp,
        ...
        AnimatedVisibility(
            ...
            Column(Modifier.fillMaxWidth())
            {
                Button(onClick = {
                    mutiSelection = false
                    transmissionScreenViewModel.mutiSelectionListVideo.forEach {
                            item ->
        ...
        )
        {
            items(
                items = downloads
                    .filter { it.type == DownloadType.VideoMainFile }
                    .groupBy { it.group }
                    .map { it.value }
                    .sortedBy { it.first().klass }
                , key = { it.first().id }
            )
            { item ->
                VideoDownloadCard(
